  ...
  hot,
  {
    ...
    async getInspectorState(query: { event: string; componentName: string }) {
      try {
        ...
        const { node, error } = findNode(tree[0], query.componentName)
        if (error) {
          rpc.onInspectorStateUpdated(query.event, { error })
          return
        }
        const state = await devtools.api.getInspectorState({
          inspectorId: COMPONENTS_INSPECTOR_ID,
        ...
        rpc.onInspectorStateUpdated(query.event, stringify(state))
      }
      catch (err) {
    ...
    async editComponentState(query: {
      ...
      try {
        ...
        const { node, error } = findNode(tree[0], query.componentName)
        if (error) {
          rpc.onEditComponentStateDone(query.event, { success: false, error })
          return
        }
        ...
        rpc.onEditComponentStateDone(query.event, { success: true })
      }
      catch (err) {
    ...
    async highlightComponent(query: { componentName: string; event: string }) {
      try {
        ...
        const { node, error } = findNode(tree[0], query.componentName)
        if (error) {
          rpc.onHighlightComponentDone(query.event, { success: false, error })
          return
        }
        if (highlightTimer) clearTimeout(highlightTimer)
        ;(devtools as any).ctx.hooks.callHook('componentHighlight', { uid: node.id })
        highlightTimer = setTimeout(() => {
          ;(devtools as any).ctx.hooks.callHook('componentUnhighlight')
        }, 5000)
        rpc.onHighlightComponentDone(query.event, { success: true })
      }
      catch (err) {
    ...
    async scrollToComponent(query: { componentName: string; event: string }) {
      try {
        ...
        const { node, error } = findNode(tree[0], query.componentName)
        if (error) {
          rpc.onScrollToComponentDone(query.event, { success: false, error })
          return
        }
        // Highlight the component (renders an overlay at its DOM position)
        if (highlightTimer) clearTimeout(highlightTimer)
        ;(devtools as any).ctx.hooks.callHook('componentHighlight', { uid: node.id })
        highlightTimer = setTimeout(() => {
          ;(devtools as any).ctx.hooks.callHook('componentUnhighlight')
        ...
        rpc.onScrollToComponentDone(query.event, { success: true })
      }
      catch (err) {
